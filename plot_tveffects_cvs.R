plot_tveslope <- function (objTVeff, objCeff, objTVeff2, lag=FALSE, cut_time = NULL, ccol, tvcol,conf.int.const=c("TRUE", "FALSE"),
  ...
  matplot(tv_effect[, 1], cbind(tv_effect[, -1], const_effect[, -1]), type = "n",
          xlab = "Time (years)", 
          ylab = if (scale %in% c("HR", "slope")) expression("Value + Slope of GPI("~t[w]* ~ X *")") else expression("Value of GPI("~t[w]* ~ X *")") )
